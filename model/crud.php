...
class crud {    
 ...
 function inserir_Usuario($nome, $endereco, $cpf, $telefone, $email, $horario, $crianca, $nascimento, $mensagem){
   ...
   $pegaEmail = mysqli_query($conn, "SELECT * FROM cadastr WHERE  responsal = '$nome' email ");	
   ...
   $sql = "INSERT INTO cadastr (nome, endereco, cpf, telefone, email, horario, crianca, nascimento, mensagem) values('$nome', '$endereco', '$cpf', '$telefone', '$email',  '$horario', '$crianca', '$nascimento', '$mensagem')";
    ...
    echo"<script language='javascript' type='text/javascript'>alert('Usuário Cadastrado com sucesso');window.location.href='../View/contato.html';</script>";
  ...
  function inserir_Usuari($nome, $endereco, $cpf, $telefone, $email, $senha, $confirma, $crianca, $nascimento, $mensagem){ 

    $conn= conectar();   
 
    $sql = "INSERT INTO Cadastro1 (nome,endereco,cpf,telefone,email,senha,confirma_senha,crianca,nascimento,mensagem) values('$nome', '$endereco', '$cpf', '$telefone', '$email', '$senha', '$confirma', '$crianca', '$nascimento', '$mensagem')";
